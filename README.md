...
| 🛠️ **Dev toolchain** | `acc` (compile = transpile→lint→bundle→minify), `aminify` (Lua-aware minifier), `abundle` (require-graph linker), `atpl` (compound-assignment transpiler), `alint` (**scope-aware** linter on a real Lua AST), `afmt` (AST-based code formatter), `atest`, `arepl` |
