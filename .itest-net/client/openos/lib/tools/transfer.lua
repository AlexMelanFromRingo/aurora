...
-- Aurora patch: stock areEqual asserted (and so crashed `cp -u`) when either
-- file could not be opened. Now an unopenable file simply means "not equal", so
-- the copy proceeds instead of aborting. Behaviour is otherwise identical.
local function areEqual(path1, path2)
  local f1 = fs.open(path1, "rb")
  if not f1 then return false end
  local f2 = fs.open(path2, "rb")
  if not f2 then f1:close(); return false end
  local result = true
